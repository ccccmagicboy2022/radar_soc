 ...
 */
__STATIC_INLINE void csi_vic_set_prio(int32_t IRQn, uint32_t irq_priority)
{
    uint8_t nlbits = (CLIC->CLICINFO & CLIC_INFO_CLICINTCTLBITS_Msk) >> CLIC_INFO_CLICINTCTLBITS_Pos;
    CLIC->CLICINT[IRQn].CTL = (CLIC->CLICINT[IRQn].CTL & (~CLIC_INTCFG_PRIO_Msk)) | (irq_priority << (8 - nlbits));
}
...
__STATIC_INLINE void set_shv_irq(int32_t IRQn)
{
    CLIC->CLICINT[IRQn].ATTR |= CLIC_INTATTR_SHV_Msk;
}

__STATIC_INLINE void set_rising_edge_irq(int32_t IRQn)
{
    CLIC->CLICINT[IRQn].ATTR = (CLIC->CLICINT[IRQn].ATTR & (~CLIC_INTATTR_TRIG_Msk)) | (1 << CLIC_INTATTR_TRIG_Pos);
}

__STATIC_INLINE void set_prio_grouping(int32_t a)
{
    CLIC->CLICCFG = (a << 1) + 1;
}
